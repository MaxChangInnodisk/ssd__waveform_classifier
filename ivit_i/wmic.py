...
def get_name_from_ismart(idx):
    command = r"iSMART_6.4.18\\iSMART.exe -d " + str(idx)
...
def get_os_product():
    ...
    """
    command = "wmic /namespace:\\\\root\\microsoft\\windows\\storage path msft_disk WHERE \"BootFromDisk='true' and IsSystem='true'\" get Number"
    p = sp.run(command, shell=True, capture_output=True, text=True)
    return [
        get_name_from_ismart(line.strip()) for line in p.stdout.split("\n")[1:] if line
    ]
