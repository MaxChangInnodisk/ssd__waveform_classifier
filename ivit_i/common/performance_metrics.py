...
class Metric:
    ...
    def paint_metrics(self, frame:ndarray, position:Tuple[int, int]=(15, 30), font_scale:float=0.75, color:Tuple[int, int, int]=(200, 10, 10), thickness:int=2):
        """ Draw performance stats (Latency, FPS) over frame """
        if self.get_latency() != -1:
            put_highlighted_text(frame, "Latency: {:.5f} ms".format(self.get_latency()),
                                 position, cv2.FONT_HERSHEY_COMPLEX, font_scale, color, thickness)
        if self.get_fps()  != -1:
            put_highlighted_text(frame, "FPS: {:.1f}".format(self.get_fps()),
    ...
    def log_total(self) -> None:
        ...
        log.info("\tLatency: {:.5f} ms".format(self.get_latency() ) if self.get_latency()  is not None else "\tLatency: N/A")
        log.info("\tFPS: {:.1f}".format(self.get_fps() ) if self.get_fps() is not None else "\tFPS: N/A")
...
if __name__ == "__main__":
    ...
    print('Latency: {}, FPS: {}'.format(
        test.get_latency(), test.get_fps()
    ))