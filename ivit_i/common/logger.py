...
def ivit_logger(
    ...
    log_folder:str="./logs") -> logging.Logger:
    ...
    formatter = logging.Formatter( "%(asctime)s [%(levelname)-.4s] %(message)s (%(filename)s:%(lineno)s)", "%y-%m-%d %H:%M:%S")
    ...
    file_handler = RotatingFileHandler(
        log_path, mode=write_mode, encoding='utf-8', maxBytes=5*1024*1024, 
        backupCount=1, delay=0)
