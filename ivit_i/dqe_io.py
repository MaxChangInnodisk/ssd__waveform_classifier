...
class DqeProcess:
    ...
    def __init__(self, module_name:str=None, module_path: str=None) -> None:
        ...
        if not "process" in dir(self.module):
...
class DqeInput:
    ...
    def __default_process(self, frame:np.ndarray, region:list = [55, 116, 669, 428]) -> np.ndarray:
    ...
    def _check_file(self, image_path: str) -> None:
        ...
        if not os.path.isfile(image_path):
            raise TypeError("{}: Except the path is a file. ({})".format(
                self.__name__,
                image_path
            ))
        return image_path
